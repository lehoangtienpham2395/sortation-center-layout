...
# Query all detailed active non-Outbound orders for Today (2026-08-01) - EXCLUDING Bưu Cục Nộp
cur.execute('''
    SELECT 
...
formatted_rows = []
for r in raw_rows:
    track, next_raw, sc, st_sys, wt, op_d, cr_t, pk_t, arr_t, inb_t = r
...
fp_all = os.path.join(ARTIFACT_DIR, 'danh_sach_tat_ca_10773_don_hang_volume_v3_01082026.csv')
fp_top10 = os.path.join(ARTIFACT_DIR, 'danh_sach_top10_chi_tiet_don_hang_v3_01082026.csv')
...
print(f"Exported v3 ALL CSV: {fp_all} ({len(formatted_rows)} rows)")
print(f"Exported v3 Top 10 CSV: {fp_top10} ({len(top10_rows)} rows)")
conn.close()
