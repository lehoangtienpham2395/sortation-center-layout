...
def compute_kpi(snap_date: str) -> dict:
    ...
    return {
        ...
        "inbound_orders":     int(inbound_total),
        "inbound_weight_ton": float(inbound_weight),
        "forecast_total":     int(forecast_total),
        "forecast_weight_ton":float(forecast_weight),
        "shuttle":            int(len(shuttle_df)),
        "shuttle_weight":     float(round(shuttle_df['wt_kg'].sum() / 1000.0, 3)) if len(shuttle_df) else 0.0,
        "linehaul":           int(len(linehaul_df)),
        "linehaul_weight":    float(round(linehaul_df['wt_kg'].sum() / 1000.0, 3)) if len(linehaul_df) else 0.0,
        "rot_hom_truoc":      int(len(backlog_df)),
        "rot_hom_nay":        int(len(ops_today)),
        "linehaul_bn_hub":    int(len(linehaul_df)),
