...
# Query all un-outbounded BN HUB forecast orders for today (2026-08-20) and active window
query = """
    SELECT 
    ...
    WHERE outbound_scandate IS NULL
      AND (
          ...
          OR (dispatch_code IS NOT NULL AND (
              ...
              dispatch_code LIKE 'HA%' OR dispatch_code LIKE 'HNI%' OR dispatch_code LIKE 'BNI%' OR
              dispatch_code LIKE 'HPG%' OR dispatch_code LIKE 'PTH%' OR dispatch_code LIKE 'NBI%'
      ...
      AND (status_sys IS NULL OR (status_sys NOT ILIKE '%hủy%' AND status_sys NOT ILIKE '%cancel%' AND status_sys NOT ILIKE '%da huy%'))
      AND operation_date_created::date >= ('2026-08-20'::date - INTERVAL '15 days')
...
# Format all object / datetime columns to clean string without timezone
...
print(f"📊 Tổng số đơn BN HUB Dự báo: {total_orders:,} đơn")
print(f"⚖️ Tổng khối lượng tính cước: {total_kg:,.2f} Kg ({total_ton:,.3f} Tấn)")

# Base project dir
base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
excel_filename = f"DS_Don_BN_HUB_DuBao_2026-08-20_{total_orders}.xlsx"
csv_filename = f"DS_Don_BN_HUB_DuBao_2026-08-20_{total_orders}.csv"
