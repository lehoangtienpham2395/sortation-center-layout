...
def batch_upsert_shipments(records, batch_size=2000):
    ...
    query = f"""
        ...
        ON CONFLICT (waybillno) DO UPDATE SET
            ...
            status_order = CASE WHEN COALESCE(EXCLUDED.outbound_scandate, shipments.outbound_scandate) IS NOT NULL THEN 'Đã rời HUB' ELSE EXCLUDED.status_order END,
            is_active = CASE WHEN COALESCE(EXCLUDED.outbound_scandate, shipments.outbound_scandate) IS NOT NULL THEN 0 ELSE EXCLUDED.is_active END,
            is_backlog = CASE WHEN COALESCE(EXCLUDED.outbound_scandate, shipments.outbound_scandate) IS NOT NULL THEN 0 ELSE EXCLUDED.is_backlog END,
