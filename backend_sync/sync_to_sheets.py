...
def run_once(session, token_mgr, rebuild_days=None):
    ...
    # Load active records from PostgreSQL
    db_records = {}
    init_db()
    try:
        conn = psycopg2.connect(**DB_CONN_PARAMS)
    ...
    for r in inbound_raw:
        ...
        # Lấy weight từ API: ưu tiên 'weight', rồi 'settlementWeight', rồi 'bulkWeight'
        try:
    ...
    for wb, info in inbound_max.items():
        ...
        if not rec['inbound_scanDate'] or info['time'] > rec['inbound_scanDate']:
            ...
            rec['changed'] = True
        # Cập nhật weight nếu chưa có (hoặc vẫn là 0) và inbound scan có giá trị
        if rec['weight'] == 0.0 and info['weight'] > 0.0:
