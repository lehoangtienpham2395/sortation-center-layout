...
def sync_postgre_to_dashboard():
    ...
    def is_linehaul_item(st, pk, status):
        ...
        return pk_u.startswith(('BN HUB', 'HN ', 'HD ', 'HY ')) or st_u.startswith(('BN HUB', 'HN ', 'HD ', 'HY '))
        ...
        for h_d in past_dates:
            cur_h.execute("""
                SELECT 
                    ...
                    (SELECT COUNT(*) FROM enriched.dispatch_enriched WHERE COALESCE(op_date_pickup::date, operation_date_created::date) = %s::date AND NOT (pickup_station LIKE 'BN HUB%%' OR pickup_station LIKE 'HN %%' OR pickup_station LIKE 'HD %%' OR pickup_station LIKE 'HY %%' OR next_station LIKE 'BN HUB%%' OR next_station LIKE 'HN %%' OR next_station LIKE 'HD %%' OR next_station LIKE 'HY %%')) as shuttle_cnt,
                    (SELECT COALESCE(SUM(orders_weight), 0) / 1000.0 FROM enriched.dispatch_enriched WHERE COALESCE(op_date_pickup::date, operation_date_created::date) = %s::date AND NOT (pickup_station LIKE 'BN HUB%%' OR pickup_station LIKE 'HN %%' OR pickup_station LIKE 'HD %%' OR pickup_station LIKE 'HY %%' OR next_station LIKE 'BN HUB%%' OR next_station LIKE 'HN %%' OR next_station LIKE 'HD %%' OR next_station LIKE 'HY %%')) as shuttle_wt,
                    (SELECT COUNT(*) FROM enriched.dispatch_enriched WHERE COALESCE(op_date_pickup::date, operation_date_created::date) = %s::date AND (pickup_station LIKE 'BN HUB%%' OR pickup_station LIKE 'HN %%' OR pickup_station LIKE 'HD %%' OR pickup_station LIKE 'HY %%' OR next_station LIKE 'BN HUB%%' OR next_station LIKE 'HN %%' OR next_station LIKE 'HD %%' OR next_station LIKE 'HY %%')) as linehaul_cnt,
                    (SELECT COALESCE(SUM(orders_weight), 0) / 1000.0 FROM enriched.dispatch_enriched WHERE COALESCE(op_date_pickup::date, operation_date_created::date) = %s::date AND (pickup_station LIKE 'BN HUB%%' OR pickup_station LIKE 'HN %%' OR pickup_station LIKE 'HD %%' OR pickup_station LIKE 'HY %%' OR next_station LIKE 'BN HUB%%' OR next_station LIKE 'HN %%' OR next_station LIKE 'HD %%' OR next_station LIKE 'HY %%')) as linehaul_wt;
