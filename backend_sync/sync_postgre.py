        ...
        for h_d in past_dates:
            ...
            cur_h.execute("""
                SELECT 
                    ...
                    (SELECT COUNT(*) FROM enriched.dispatch_enriched WHERE COALESCE(op_date_pickup::date, operation_date_created::date) = %s::date AND NOT ((pickup_station LIKE 'BN HUB%%' OR pickup_station LIKE 'HN %%' OR pickup_station LIKE 'HD %%' OR pickup_station LIKE 'HY %%' OR next_station LIKE 'BN HUB%%' OR next_station LIKE 'HN %%' OR next_station LIKE 'HD %%' OR next_station LIKE 'HY %%') AND status_sys IN ('Transporting', 'Arrival', 'Created'))) as shuttle_cnt,
                    (SELECT COALESCE(SUM(orders_weight), 0) / 1000.0 FROM enriched.dispatch_enriched WHERE COALESCE(op_date_pickup::date, operation_date_created::date) = %s::date AND NOT ((pickup_station LIKE 'BN HUB%%' OR pickup_station LIKE 'HN %%' OR pickup_station LIKE 'HD %%' OR pickup_station LIKE 'HY %%' OR next_station LIKE 'BN HUB%%' OR next_station LIKE 'HN %%' OR next_station LIKE 'HD %%' OR next_station LIKE 'HY %%') AND status_sys IN ('Transporting', 'Arrival', 'Created'))) as shuttle_wt,
                    (SELECT COUNT(*) FROM enriched.dispatch_enriched WHERE COALESCE(op_date_pickup::date, operation_date_created::date) = %s::date AND ((pickup_station LIKE 'BN HUB%%' OR pickup_station LIKE 'HN %%' OR pickup_station LIKE 'HD %%' OR pickup_station LIKE 'HY %%' OR next_station LIKE 'BN HUB%%' OR next_station LIKE 'HN %%' OR next_station LIKE 'HD %%' OR next_station LIKE 'HY %%') AND status_sys IN ('Transporting', 'Arrival', 'Created'))) as linehaul_cnt,
                    (SELECT COALESCE(SUM(orders_weight), 0) / 1000.0 FROM enriched.dispatch_enriched WHERE COALESCE(op_date_pickup::date, operation_date_created::date) = %s::date AND ((pickup_station LIKE 'BN HUB%%' OR pickup_station LIKE 'HN %%' OR pickup_station LIKE 'HD %%' OR pickup_station LIKE 'HY %%' OR next_station LIKE 'BN HUB%%' OR next_station LIKE 'HN %%' OR next_station LIKE 'HD %%' OR next_station LIKE 'HY %%') AND status_sys IN ('Transporting', 'Arrival', 'Created'))) as linehaul_wt;
